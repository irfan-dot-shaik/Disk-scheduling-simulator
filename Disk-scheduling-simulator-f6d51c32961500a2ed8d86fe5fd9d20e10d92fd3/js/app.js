...
(() => {
  ...
  const inputNstep      = $("#input-nstep");
  const nstepGroup      = $("#nstep-group");
  ...
  function nstepscan(requests, head, diskSize, direction, n = 4) {
    const sequence = [head];
    let total = 0, current = head, currentDir = direction;
    const pending = [...requests];
    while (pending.length > 0) {
      ...
      if (currentDir === "right") {
        run = [...right];
        if (left.length > 0) {
          run.push(diskSize - 1); run.push(...left.reverse()); currentDir = "left";
        } else if (right.length > 0) {
          run.push(diskSize - 1); currentDir = "left";
        }
      } else {
        run = [...left.reverse()];
        if (right.length > 0) {
          run.push(0); run.push(...right); currentDir = "right";
        } else if (left.length > 0) {
          run.push(0); currentDir = "right";
        }
      }
      for (const c of run) {
        if (c !== current) { total += Math.abs(c - current); current = c; sequence.push(c); }
      }
    }
    return { sequence, total, name: "N-Step SCAN" };
  }

  function fscan(requests, head, diskSize, direction) {
    const left = requests.filter(r => r < head).sort((a, b) => a - b);
    const right = requests.filter(r => r >= head).sort((a, b) => a - b);
    let run;
    if (direction === "right") {
  ...
  function runAlgorithm(algo, requests, head, diskSize, direction, n = 4) {
    switch (algo) {
      ...
      case "nstepscan": return nstepscan(requests, head, diskSize, direction, n);
      case "fscan": return fscan(requests, head, diskSize, direction);
  ...
  function getInputs() {
    ...
    const n = parseInt(inputNstep.value, 10) || 4;
    return { requests, head, diskSize, direction, algo, n };
  }
  ...
  function drawComparison(requests, head, diskSize, direction, n) {
    const algos = ["fcfs", "sstf", "scan", "cscan", "look", "clook", "nstepscan", "fscan"];
    const names = ["FCFS", "SSTF", "SCAN", "C-SCAN", "LOOK", "C-LOOK", "N-Step SCAN", "F-SCAN"];
    const colors = ["#f43f5e", "#10b981", "#3b82f6", "#f59e0b", "#8b5cf6", "#ec4899", "#14b8a6", "#84cc16"];
    const results = algos.map(a => runAlgorithm(a, requests, head, diskSize, direction, n));
  ...
  function exportCSV() {
    ...
    const nValue = parseInt(inputNstep.value, 10) || 4;
    const algoDisplay = currentResult.name === "N-Step SCAN" ? `N-Step SCAN (N=${nValue})` : currentResult.name;
    const rows = [
  ...
  selectAlgorithm.addEventListener("change", () => {
    const algo = selectAlgorithm.value;
    dirGroup.classList.toggle("collapsed", !["scan", "look", "nstepscan", "fscan"].includes(algo));
    nstepGroup.classList.toggle("collapsed", algo !== "nstepscan");
  });
  const initialAlgo = selectAlgorithm.value;
  dirGroup.classList.toggle("collapsed", !["scan", "look", "nstepscan", "fscan"].includes(initialAlgo));
  if (nstepGroup) nstepGroup.classList.toggle("collapsed", initialAlgo !== "nstepscan");

  btnSimulate.addEventListener("click", () => {
    const inp = getInputs();
    if (!inp) return;
    const result = runAlgorithm(inp.algo, inp.requests, inp.head, inp.diskSize, inp.direction, inp.n);
  ...
  btnCompare.addEventListener("click", () => {
    ...
    drawSeekChart(result.sequence, inp.diskSize, result.name);
    animateDiskHead(result.sequence, inp.diskSize);
    drawComparison(inp.requests, inp.head, inp.diskSize, inp.direction, inp.n);
  });
  ...
  btnReset.addEventListener("click", () => {
    ...
    nstepGroup.classList.add("collapsed");
